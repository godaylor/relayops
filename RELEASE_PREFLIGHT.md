...
Mobile shell and OpenAPI/Helm guidance fixes are verified; see FINAL_AUDIT.md for current evidence. The original tables below describe the previous run and images. New local images use :audit-fix; exact IDs, refreshed SBOM/licenses and Trivy 0.74.0 reports (0 reported vulnerabilities each) are in artifacts/audit-fix/images.json. Full browser regression: 12/12 including all six mobile screens in RU/EN. Main :local runtime on 32000 was not replaced; disposable verify stack stopped. Source candidate includes this follow-up and FINAL_AUDIT.md. No push or deploy. Manual AT is explicitly deferred; clean GitHub security runs and Pages deployment are downstream of the public-repository push.
...
- Origin is still `https://github.com/usekaneo/kaneo.git`, **not a RelayOps publication destination**. The authenticated GitHub owner is `godaylor`; `publication` points to `https://github.com/godaylor/relayops.git`; the intended free demo host is GitHub Pages. Publishing workflows remain fail-closed until the push completes and repository variables are set.
...
| Local release checks | `pnpm release:check`, version 2.22.0 consistency check, immutable upstream MIT comparison and separate RelayOps MIT/NOTICE checks pass. Demo push was approved; terminal HTTPS egress is currently unavailable |
...
- Independent local source scans were added during publication preparation: Gitleaks 8.30.1 reported zero leaks in the exact staged tree and all six commits; Semgrep 1.175.1 ERROR findings were fixed. GitHub-hosted CodeQL/dependency-review remain downstream of the push; manual assistive-technology review is deferred by owner.
...
- The prepared public repository exists under authenticated owner `godaylor`; push through the separate `publication` remote and never publish to upstream `origin`.
