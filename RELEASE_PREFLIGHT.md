...
- Independent local source scans were added during publication preparation: Gitleaks 8.30.1 reported zero leaks in the exact staged tree and the publication range; Semgrep 1.175.1 ERROR findings were fixed. GitHub-hosted Security run `34479944174` completed successfully for the published HEAD; manual assistive-technology review is deferred by owner.
...
- The public repository is `https://github.com/godaylor/relayops`; `publication/main` contains the current branch, while upstream `origin` remains untouched. Pages run `34480271657` completed successfully and published `https://godaylor.github.io/relayops/`.
